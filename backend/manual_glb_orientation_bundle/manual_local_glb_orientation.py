...
def deg_to_quaternion_x(deg: float) -> dict[str, float]:
    """Convert an X-axis rotation (degrees) to a Three.js quaternion [x,y,z,w]."""
    rad = math.radians(deg)
    return {
        "x": round(math.sin(rad / 2), 10),
        "y": 0.0,
        "z": 0.0,
        "w": round(math.cos(rad / 2), 10),
    }
...
def deg_to_quaternion_z(deg: float) -> dict[str, float]:
    """Convert a Z-axis rotation (degrees) to a Three.js quaternion [x,y,z,w]."""
    rad = math.radians(deg)
    return {
        "x": 0.0,
        "y": 0.0,
        "z": round(math.sin(rad / 2), 10),
        "w": round(math.cos(rad / 2), 10),
    }
...
def build_preview_payload(
    *,
    ...
    # Q_yaw × (Q_x × Q_z) matches Three.js Euler XYZ applied inside a yaw group
    q_yaw = deg_to_quaternion_y(float(offset))
    q_x = deg_to_quaternion_x(float(tilt_x))
    q_z = deg_to_quaternion_z(float(tilt_z))
    quat = normalize_quaternion(multiply_quaternions(q_yaw, multiply_quaternions(q_x, q_z)))
...
def build_result_payload(
    *,
    ...
    # Three.js hierarchy: baseGroup → yawGroup (Y) → tiltGroup (X, Z) → model
    q_offset = deg_to_quaternion_y(float(offset))
    q_x = deg_to_quaternion_x(float(tilt_x))
    q_z = deg_to_quaternion_z(float(tilt_z))
    tilt_quat = multiply_quaternions(q_x, q_z)
    correction_quat = normalize_quaternion(multiply_quaternions(q_offset, tilt_quat))
    corrected_quat = normalize_quaternion(multiply_quaternions(base_quat, correction_quat))
    return {
        "object_id": obj_id,
        "name": obj.get("name"),
